...
import static org.wildfly.swarm.spi.api.Configurable.bool;
...
public class UndertowFraction extends Undertow<UndertowFraction> implements Fraction {
    ...
    private Configurable<Integer> httpPort = integer("swarm.http.port", DEFAULT_HTTP_PORT);
    private Configurable<Integer> httpsPort = integer("swarm.https.port", DEFAULT_HTTPS_PORT);
    private Configurable<Integer> ajpPort = integer("swarm.ajp.port", DEFAULT_AJP_PORT);
     ...
     */
    private Configurable<Boolean> enableAJP = ifAnyExplicitlySet( "swarm.ajp.enable", this.ajpPort);
