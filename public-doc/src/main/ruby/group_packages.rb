...
content = read_overview_frame("target/site/apidocs/overview-frame.html")
content[:packages] = group_packages(content[:packages], GROUPINGS)
write_content(content, "target/site/apidocs/overview-frame.html") do |f, title, packages|
  f.puts "<h3>#{title}</h3><ul>"
  packages.each {|line| f.puts(line)}
  f.puts "</ul>"
end

content = read_overview_summary("target/site/apidocs/overview-summary.html")
content[:packages] = group_packages(content[:packages], GROUPINGS)
write_content(content, "target/site/apidocs/overview-summary.html") do |f, title, packages|
