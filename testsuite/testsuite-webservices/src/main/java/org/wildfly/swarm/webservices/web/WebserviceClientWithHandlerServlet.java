 ...
 */
@WebServlet("/handled")
public class WebserviceClientWithHandlerServlet extends WebserviceClientServlet {

    @Override
    protected void configure(Binding wsBinding) {
        final List<Handler> handlerChain = wsBinding.getHandlerChain();
        handlerChain.add(new SampleSoapHandler());
        wsBinding.setHandlerChain(handlerChain);
    }
