...
[NOTE]
====
anchor:hollow-jar-limitations[]Using pre-built hollow JARs has certain limitations:
...
----
$ java -Dswarm.classpath=./h2-1.4.196.jar -jar microprofile-jpa-hollow-swarm.jar myApp.war
----

* YAML configuration files in your application that are named `project-*.yml` are not automatically applied.
You must specify them manually, for example:
+
[source,bash,options="nowrap"]
----
$ java -s project-myconfig.yml -jar microprofile-jpa-hollow-swarm.jar myApp.war
----
====
