...
If you do not know which one to use, open `project-defaults.yml` in the root directory of the WAR archive with your application.
...
swarm:
  logging:
----
. Configure a formatter (optional). Under `logging`, add a new formatter with a pattern of your choice.
...
The file stores logs in the `target` directory, and each log file is named `MY_APP.log` with the suffix `.yyyy-MM-dd`.
