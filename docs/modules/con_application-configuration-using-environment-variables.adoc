...
* All _dash/hyphen_ (-) characters must be replaced with the `DASH` string.
For example, define the `thorntail.data-sources.foo.url=<url>` property as `THORNTAIL_DATA_DASH_SOURCES_FOO_URL=<url>`.
* If the property name contains underscores, all _underscore_ (\_) characters must be replaced with the `UNDERSCORE` string.
