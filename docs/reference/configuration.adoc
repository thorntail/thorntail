...
When executing a Hollow Jar,
there's now a way to specify a JDBC Driver jar without needing to package it into the Hollow Jar.

We can use the `swarm.classpath` System property:
...
The `swarm.classpath` property can accept multiple JARs separated by `;`,
allowing multiple JDBC Drivers, or other libraries, to be added to the application module classpath.
